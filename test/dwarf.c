...
fetch_indirect_string (unsigned long offset)
{
  ...
  if (offset > section->size)
    {
      warn (_("DW_FORM_strp offset too big: 0x%lx\n"), offset);
...
get_TAG_name (unsigned long tag)
{
  switch (tag)
	...
	snprintf (buffer, sizeof (buffer), _("Unknown TAG value: 0x%lx"), tag);
	return buffer;
      }
    }
}
...
get_FORM_name (unsigned long form)
{
  switch (form)
	...
	snprintf (buffer, sizeof (buffer), _("Unknown FORM value: 0x%lx"), form);
	return buffer;
      }
    }
}
...
display_block (unsigned char *data, unsigned long length)
{
  ...
  while (length --)
    printf ("%02lx ", (unsigned long) byte_get (data++, 1));
			    ...
			    unsigned long cu_offset)
{
  ...
  while (data < end)
	...
	{
	case DW_OP_addr:
	  printf ("[0x%lx DW_OP_addr]", (unsigned long) byte_get (data, pointer_size));
	  data += pointer_size;
	  break;
	case DW_OP_deref:
	  printf ("[DW_OP_deref]");
	  break;
	case DW_OP_const1u:
	  printf ("[%lu DW_OP_const1u]", (unsigned long) byte_get (data++, 1));
	  break;
	case DW_OP_const1s:
	  printf ("[%ld DW_OP_const1s]", (long) byte_get_signed (data++, 1));
	  break;
	case DW_OP_const2u:
	  printf ("[%lu DW_OP_const2u]", (unsigned long) byte_get (data, 2));
	  data += 2;
	  break;
	case DW_OP_const2s:
	  printf ("[%ld DW_OP_const2s]", (long) byte_get_signed (data, 2));
	  data += 2;
	  break;
	case DW_OP_const4u:
	  printf ("[%lu DW_OP_const4u]", (unsigned long) byte_get (data, 4));
	  data += 4;
	  break;
	case DW_OP_const4s:
	  printf ("[%ld DW_OP_const4s]", (long) byte_get_signed (data, 4));
	  data += 4;
	  break;
	case DW_OP_const8u:
	  printf ("[%lu %lu DW_OP_const8u]", (unsigned long) byte_get (data, 4),
		  (unsigned long) byte_get (data + 4, 4));
	  data += 8;
	  break;
	case DW_OP_const8s:
	  printf ("[%ld %ld DW_OP_const8s]", (long) byte_get (data, 4),
		  (long) byte_get (data + 4, 4));
	  data += 8;
	  break;
	case DW_OP_constu:
	  printf ("[%lu DW_OP_constu]", read_leb128 (data, &bytes_read, 0));
	  data += bytes_read;
	  break;
	case DW_OP_consts:
	  printf ("[%ld DW_OP_consts]", read_leb128 (data, &bytes_read, 1));
	  data += bytes_read;
	  break;
	case DW_OP_dup:
	  printf ("[DW_OP_dup]");
	...
	case DW_OP_over:
	  ...
	  break;
	case DW_OP_pick:
	  printf ("[%ld DW_OP_pick]", (unsigned long) byte_get (data++, 1));
	  break;
	case DW_OP_swap:
	  printf ("[DW_OP_swap]");
	  break;
	...
	case DW_OP_xor:
	  ...
	  break;
	case DW_OP_bra:
	  printf ("[%ld DW_OP_bra]", (long) byte_get_signed (data, 2));
	  data += 2;
	  break;
	case DW_OP_eq:
	  printf ("[DW_OP_eq]");
	...
	case DW_OP_ne:
	  ...
	  break;
	case DW_OP_skip:
	  printf ("[%ld DW_OP_skip]", (long) byte_get_signed (data, 2));
	  data += 2;
	  break;
	...
	case DW_OP_breg30:
	case DW_OP_breg31:
            printf ("[%ld DW_OP_breg%d]", read_leb128 (data, &bytes_read, 1),op - DW_OP_breg0);
	  data += bytes_read;
	  break;

	case DW_OP_regx:
	  printf ("[%lu DW_OP_regx]", read_leb128 (data, &bytes_read, 0));
	  data += bytes_read;
	  break;
	case DW_OP_fbreg:
	  need_frame_base = 1;
	  printf ("[%ld DW_OP_fbreg]", read_leb128 (data, &bytes_read, 1));
	  data += bytes_read;
	  break;
	case DW_OP_bregx:
	  uvalue = read_leb128 (data, &bytes_read, 0);
	  data += bytes_read;
	  printf ("[%lu %ld DW_OP_bregx]", uvalue,
		  read_leb128 (data, &bytes_read, 1));
	  data += bytes_read;
	  break;
	case DW_OP_piece:
	  printf ("[%lu DW_OP_piece]", read_leb128 (data, &bytes_read, 0));
	  data += bytes_read;
	  break;
	case DW_OP_deref_size:
	  printf ("[%ld DW_OP_deref_size]", (long) byte_get (data++, 1));
	  break;
	case DW_OP_xderef_size:
	  printf ("[%ld DW_OP_xderef_size]", (long) byte_get (data++, 1));
	  break;
	case DW_OP_nop:
	  printf ("[DW_OP_nop]");
	  break;
	...
	case DW_OP_call2:
	  ...
	  printf ("[0x%lx DW_OP_call2]", (long) byte_get (data, 2) + cu_offset);
	  data += 2;
	  break;
	case DW_OP_call4:
	  /* XXX: Strictly speaking for 64-bit DWARF3 files
	     this ought to be an 8-byte wide computation.  */
	  printf ("[0x%lx DW_OP_call4]", (long) byte_get (data, 4) + cu_offset);
	  data += 4;
	  break;
	case DW_OP_call_ref:
			     ...
			     int do_loc)
{
  ...
  switch (attribute)
	...
	case DW_INL_declared_inlined:
	  ...
	  break;
	default:
	  printf (_("  (Unknown inline attribute value: 0x%lx)"), uvalue);
	  break;
	}
      break;
	...
	default:
	  if (uvalue >= DW_LANG_lo_user && uvalue <= DW_LANG_hi_user)
	    printf ("(implementation defined: 0x%lx)", uvalue);
	  else
	    printf ("(Unknown: 0x%lx)", uvalue);
	  break;
	}
      break;
      ...
      if (block_start)
	{
	  int need_frame_base;
          ...
          printf("]/ / "); // going to replace previous item
	  need_frame_base = decode_location_expression (block_start,
							pointer_size,
							uvalue,
							cu_offset);
          printf(" ["); // satisfy closure below
...
get_AT_name (unsigned long attribute)
{
  switch (attribute)
	...
	snprintf (buffer, sizeof (buffer), _("Unknown AT value: 0x%lx"),
		  attribute);
	return buffer;
      }
    }
  ...
  for (section_begin = start, unit = 0; start < end; unit++)
      ...
      while (tags < start)
	{
	  ...
	  if (entry == NULL)
	    {
          ...
          id = (unsigned long) (tags - section_begin - bytes_read);
	  if (!do_loc)
          {
              printf ("[]@0x%lx\n" // create item for new scope
                      "0x%lx <\n" // new scope for this entry
                      "\t[%s]@type\n",
                      id,id,get_TAG_name(entry->tag));
         }
	  ...
	  for (attr = entry->first_attr; attr; attr = attr->next)
          ...
          if (!do_loc)
              printf(">\n"); // end scope for this entry
 
 	  if (entry->children)
          {
 	    ++level;
            if (!do_loc)
                printf("[]@0x%lx.children 0x%lx.children<\n",id,id); // reopen scope for children of this entry
          }
 	}
    }
  ...
  for (i = first; i < num_debug_info_entries; i++)
      ...
      for (j = 0; j < debug_information [i].num_loc_offsets; j++)
	{
	  ...
	  while (1)
		...
		{
		  base_address = end;
		  printf (_("    0x%8.8lx 0x%8.8lx 0x%8.8lx (base address)\n"),
			  offset, begin, end);
		  continue;
		}
  ...
  for (i = first; i < num_debug_info_entries; i++)
      ...
      for (j = 0; j < debug_information [i].num_range_lists; j++)
	{
	  ...
	  while (1)
		...
		{
		  base_address = end;
		  printf ("    0x%8.8lx 0x%8.8lx 0x%8.8lx (base address)\n",
			  offset, begin, end);
		  continue;
		}
  ...
  while (start < end)
      ...
      if (is_eh ? (cie_id == 0) : (cie_id == DW_CIE_ID))
	{
	  ...
	  if (do_debug_frames_interp)
	    printf ("\n0x%08lx 0x%08lx 0x%08lx CIE \"%s\" cf=%d df=%d ra=%d\n",
		    (unsigned long)(saved_start - section_start), length, cie_id,
		    fc->augmentation, fc->code_factor, fc->data_factor,
		    fc->ra);
	  else
	    {
	      printf ("\n0x%08lx 0x%08lx 0x%08lx CIE\n",
		      (unsigned long)(saved_start - section_start), length, cie_id);
	      printf ("  Version:               %d\n", version);
      ...
      else
	{
	  ...
	  printf ("\n0x%08lx 0x%08lx 0x%08lx FDE cie=0x%08lx pc=0x%08lx..0x%08lx\n",
		  (unsigned long)(saved_start - section_start), length, cie_id,
      ...
      while (start < block_end)
	{
	  ...
	  switch (op)
		...
		printf ("  DW_CFA_advance_loc: %d to 0x%08lx\n",
			opa * fc->code_factor,
		...
		printf ("  DW_CFA_advance_loc1: %ld to 0x%08lx\n",
			ofs * fc->code_factor,
		...
		printf ("  DW_CFA_advance_loc2: %ld to 0x%08lx\n",
			ofs * fc->code_factor,
			fc->pc_begin + ofs * fc->code_factor);
	      fc->pc_begin += ofs * fc->code_factor;
	      break;
		...
		printf ("  DW_CFA_advance_loc4: %ld to 0x%08lx\n",
			ofs * fc->code_factor,
			fc->pc_begin + ofs * fc->code_factor);
	      fc->pc_begin += ofs * fc->code_factor;
	      break;
		...
		printf ("  DW_CFA_MIPS_advance_loc8: %ld to 0x%08lx\n",
			ofs * fc->code_factor,
			fc->pc_begin + ofs * fc->code_factor);
	      fc->pc_begin += ofs * fc->code_factor;
	      break;
