...
typedef std::function<int(Dwarf_Debug, Dwarf_Die, DIEWALK_FLAGS)> DIE_OP;
...
iterate:
    ...
    TRY(dwarf_siblingof_b(dbg, die, (flags & RDW_is_info) != 0, &sibling, &error), "retrieving dwarf_sibling");
...
int populate_type_info(Dwarf_Debug dbg, Dwarf_Die die, TYPE_INFO_LTV *type_info, CU_DATA *cu_data) {
    ...
    STRY(dwarf_die_CU_offset(die, &type_info->offset, &error), "getting CU-relative die offset");
    Dwarf_Off goff;
    ...
    while (atcnt--) {
        ...
        auto dump_attr = [&](FILE *ofile) {
            ...
            fprintf(ofile, CODE_RED "dumping attr 0x%x\n", vshort);
            Dwarf_Signed   vint;
...
{
    CU_DATA cu_data;
    auto    op = [&](Dwarf_Debug dbg, Dwarf_Die die, DIEWALK_FLAGS flags) {
        int         status  = 0;
        Dwarf_Error error   = 0;
        char *      cu_name = NULL;
        Dwarf_Off   offset;
...
int cif_dump_module(char *ofilename, LTV *module) {
    ...
    auto traverse_types = [&](LTI **lti, LTVR *ltvr, LTV **ltv, int depth, LT_TRAVERSE_FLAGS *flags) {
...
static int cif_curate_module(LTV *module, int bootstrap) {
    ...
    auto derive_symbolic_name = [&](TYPE_INFO_LTV *type_info, int post) {
        ...
        auto categorize_symbolic = [&](char *sym) {
            ...
            if (!deduped) {
                DEBUG(fprintf(stdout, "installing symbolic type_info %s (%s)\n", sym, type_info->id_str));
    ...
    std::function<int(Dwarf_Debug, Dwarf_Die, DIEWALK_FLAGS)> curate_die = [&](Dwarf_Debug dbg, Dwarf_Die die, DIEWALK_FLAGS flags) {
        std::function<int(LTV *, Dwarf_Die, int)> work_op = [&](LTV *parent, Dwarf_Die die, int depth) {  // propagates parentage through the stateless DIE_OP calls
            int            status           = 0;
            Dwarf_Error    error            = 0;
            TYPE_INFO_LTV *type_info        = NULL;
            TYPE_INFO_LTV *parent_type_info = (TYPE_INFO_LTV *) parent;

            auto read_cu_macros = [&]() {  // inspired by https://github.com/tomhughes/libdwarf/blob/master/libdwarf/checkexamples.c
                int status = 0;

                auto macro_define = [&](char *macro) {
                    int  len     = strlen(macro);
                    auto advance = [&](int adv) { macro+=adv; len-=adv; };
                ...
                auto macro_undefine = [&](char *macro) { /* fprintf(stdout,"undefine %s\n",macro); */ };
            ...
            auto child_op = [&](Dwarf_Debug dbg, Dwarf_Die die, DIEWALK_FLAGS flags) { return work_op(&type_info->ltv, die, depth + 1); };

            auto link2parent = [&](char *name) {
                int status = 0;
                switch (type_info->tag) {
                    case 0:  // rejected
                        break;
            ...
            if (!(type_info = (TYPE_INFO_LTV *) LT_get(index[is_cu], offset_str, HEAD, KEEP))) {  // may have been curated previously
                ...
                if (type_info->tag != DW_TAG_compile_unit || LT_get(module, name, HEAD, KEEP)) {  // only traverse listed CU's siblings
                    ...
                    if ((type_info->flags & TYPEF_IS_DECL) && (type_info->flags & TYPEF_SIGNATURE)) {
                        ...
                        DWARF_ALIAS(alias, type_info->sig8);
                        STRY(!LT_put(aliases, alias, TAIL, &type_info->ltv), "aliasing type info %s with %s", type_info->id_str, alias);
                        DEBUG(fprintf(stdout, "--- aliasing %s sig %s\n", type_info->id_str, alias));
                    }
    ...
    auto resolve_bases = [&](LTI **lti, LTVR *ltvr, LTV **ltv, int depth, LT_TRAVERSE_FLAGS *flags) {
    ...
    auto remove_die_names = [&](LTI **lti, LTVR *ltvr, LTV **ltv, int depth, LT_TRAVERSE_FLAGS *flags) {
        if (!listree_acyclic(lti, ltvr, ltv, depth, flags)) {
            if ((*flags & LT_TRAVERSE_LTV) && (*ltv)->flags & LT_TYPE)
    ...
    auto resolve_meta = [&](LTI **lti, LTVR *ltvr, LTV **ltv, int depth, LT_TRAVERSE_FLAGS *flags) {
        if ((*flags & LT_TRAVERSE_LTV) && (*ltv)->flags & LT_TYPE) {
            TYPE_INFO_LTV *type_info = (TYPE_INFO_LTV *) (*ltv);
            LTV *          base_info = NULL;
    ...
    auto resolve_types = [&](LTI **lti, LTVR *ltvr, LTV **ltv, int depth, LT_TRAVERSE_FLAGS *flags) {
        if (!listree_acyclic(lti, ltvr, ltv, depth, flags)) {
            if (((*flags) & LT_TRAVERSE_LTV) && (*ltv)->flags & LT_TYPE) {
    ...
    auto resolve_symbols = [&](char *filename) {
        char *f = bootstrap ? NULL : filename;
        if (!(dlhandle = dlopen(f, RTLD_LAZY | RTLD_GLOBAL | RTLD_NODELETE | RTLD_DEEPBIND)))
            fprintf(stderr, "failed while dlopen'ing %s; continuing without resolving global functions/variables\n", dlerror());
...
extern void print_type(FILE *ofile, LTV *ltv, char *prefix) {
    int old_show_ref = show_ref;
    show_ref         = 1;
    if (prefix)
        fprintf(ofile, "%s", prefix);
    print_ltv(ofile, NULL, ltv, NULL, 2);
...
LTV *cif_ffi_prep(LTV *lambda) {
    auto collate_child_ffi_types = [&](LTV *ltv, int tag, int *count, ffi_type ***child_types) {
        int   status   = 0;
        int   largest  = 0;
        ...
        if (*count) {
            ...
            auto     get_child_ffi_type = [&](CLL *lnk) {
                int            status     = 0;
    ...
    auto pre = [&](LTI **lti, LTVR *ltvr, LTV **ltv, int depth, LT_TRAVERSE_FLAGS *flags) {
        int status = 0;
    ...
    auto post = [&](LTI **lti, LTVR *ltvr, LTV **ltv, int depth, LT_TRAVERSE_FLAGS *flags) {
        int status = 0;
        if ((*flags & LT_TRAVERSE_LTV)) {
            char *name = attr_get((*ltv), TYPE_SYMB);
            if ((*ltv)->flags & LT_TYPE) {
                ...
                if (!LT_get((*ltv), FFI_TYPE, HEAD, KEEP)) {
                    switch (type_info->tag) {
                        case DW_TAG_union_type:
                        case DW_TAG_structure_type:
                        case DW_TAG_class_type: {  // complex
...
int cif_args_marshal(LTV *lambda, int dir, CIF_MARSHAL_OP marshal) {
    int  status      = 0;
    auto marshal_arg = [&](CLL *lnk) {
        int   status   = 0;
...
LTV *cif_coerce_i2c(LTV *ltv, LTV *type) {
    ...
    */
    ...
    auto  match     = [&](char *key) { return (int) !strcmp(key, type_name); };
